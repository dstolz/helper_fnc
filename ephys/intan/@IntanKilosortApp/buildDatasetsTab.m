...
g = uigridlayout(obj.TabDatasets, [3 5]);
g.RowHeight    = {'fit', 'fit', '1x'};
g.ColumnWidth  = {'fit', '1x', 'fit', 'fit', 'fit'};
...
obj.ScanStatusLabel.Layout.Row = 2; obj.ScanStatusLabel.Layout.Column = [1 5];

% Row 3: datasets table
obj.DatasetsTable = uitable(g);
obj.DatasetsTable.Layout.Row = 3; obj.DatasetsTable.Layout.Column = [1 5];
obj.DatasetsTable.ColumnName = {'Select', 'Name', 'Acq date', '# files', ...
    '# chan', 'Fs (Hz)', 'Duration (min)', 'Probe', 'Exclude', 'Bin'};
obj.DatasetsTable.ColumnEditable = [true false false false false false false false false false];
obj.DatasetsTable.CellSelectionCallback = @(~,evt) obj.onDatasetCellSelection(evt);
obj.DatasetsTable.ColumnWidth = {50, 'auto', 130, 60, 60, 80, 100, 60, 90, 50};
end
